...
Now, the transformations operate on dataframes, which we obtained through interacting with S3. As mentioned, we would like to remove that dependency and focus solely on the transformations. If(2) the transformations would no longer get data from files, how do we still feed it data?
